...
use crate::exec::{DispatchError};
use crate::exec::{Ext, StorageKey, ExecResult, ExecError};
	...
	E: Ext + 'a
{
	...
	{
		// FIXME: implement ensure!
		// ensure!(len <= self.ext.schedule().limits.max_memory_size(), Error::OutOfBounds);
...
define_env!(Env, <E: Ext>,
	...
	[seal0] seal_value_transferred(ctx, out_ptr: u32, out_len_ptr: u32) => {
		...
		*/
		println!("Unsupported host function seal_value_transferred() called");
		Ok(())
	},
	...
	[seal0] seal_call_chain_extension(
		...
		*/
		println!("Unsupported host function seal_call_chain_extension() called");
        Ok(0)
	},
	...
	[seal0] seal_rent_params(ctx, out_ptr: u32, out_len_ptr: u32) => {
		...
		*/
		println!("Unsupported host function seal_rent_params() called");
        Ok(())
	},
);
